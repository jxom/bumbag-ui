---
name: Blockquote
route: /typography/blockquote
menu: Typography
---

import { Playground } from 'docz';
import PropsTable from '../_docs/components/PropsTable';
import { Box } from '../primitives/index';
import Text from '../Text';
import Paragraph from '../Paragraph';
import {Blockquote} from './Blockquote';

# Blockquote

## Import

`import { Blockquote } from 'fannypack'`

## Basic Usage

`<Blockquote>` renders a `<blockquote>` element with reset styles and a left border.

<Playground>
  <Blockquote>
    <Paragraph marginBottom="4u">"Out here, survival is the name of the game. Only it's not a game; it's deadly serious."</Paragraph>
    <Text use="cite">- Russell Coight</Text>
  </Blockquote>
  <Blockquote backgroundColor="whitesmoke" marginTop="4u">
    <Paragraph marginBottom="4u">"Out here, survival is the name of the game. Only it's not a game; it's deadly serious."</Paragraph>
    <Text use="cite">- Russell Coight</Text>
  </Blockquote>
</Playground>

## Border styling

The left border can be modified using the primitive style props:

<Playground>
  <Blockquote borderColor="blue">
    <Paragraph marginBottom="4u">"Out here, survival is the name of the game. Only it's not a game; it's deadly serious."</Paragraph>
    <Text use="cite">- Russell Coight</Text>
  </Blockquote>
</Playground>

Or you can hide the border:

<Playground>
  <Blockquote border="none">
    <Paragraph marginBottom="4u">"Out here, survival is the name of the game. Only it's not a game; it's deadly serious."</Paragraph>
    <Text use="cite">- Russell Coight</Text>
  </Blockquote>
</Playground>

## Props

<PropsTable component={Blockquote} />

## Theming

### Schema

```jsx
{
  Blockquote: {
    base: css | Object
  }
}
```

[Learn more about theming](/styling#theming)
