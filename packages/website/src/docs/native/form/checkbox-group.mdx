...
import { CheckboxGroup as BNCheckboxGroup, CheckboxGroupField as BNCheckboxGroupField } from 'bumbag-native';
...
```jsx
import { CheckboxGroup, CheckboxGroupField } from 'bumbag-native'
```
...
A CheckboxGroup can use different colors (as per palette) such as `secondary`, `danger`, `success` and `warning`.
...
```function-live
function Example() {
  const [value, setValue] = React.useState(['overcast', 'sunny']);
...
{`(Pick<any, string
  | number
  | symbol> & ViewProps & LocalBoxProps & { active?: boolean; focus?: boolean; hover?: boolean; hoveractive?: boolean; } & LocalCheckboxProps & { ...; })[]`}
</Code>
...
**<Code fontSize="150" marginRight="major-1">setProps</Code>**  
...
<Theme
  ...
  overrides={[
    ...
    'CheckboxGroup.Item.styles.base'
  ]} />

### CheckboxGroupField

<Theme
  component={BNCheckboxGroupField}
  label="Weather"
  options={[
    { label: 'Sunny', value: 'sunny' },
    { label: 'Windy', value: 'windy' },
  ...
  overrides={[
    'CheckboxGroupField.styles.base'
  ]} />
