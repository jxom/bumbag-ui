...
If you wish to upgrade incrementally, you can install the `fannypack-v5` package:

```
npm install fannypack-v5
```

And then, set up Fannypack v5 in your app root:

```jsx
// v4 Fannypack
import { ThemeProvider } from 'fannypack';
// v5 Fannypack
import { Provider as FannypackProvider } from 'fannypack-v5';
...
const App = () => (
  <ThemeProvider theme={oldTheme}>
    <FannypackProvider theme={newTheme}>
      Hello world!
    </FannypackProvider>
  </ThemeProvider>
);
...
We have moved the `HighlightedCode` component to `fannypack-addon-highlighted-code` to save some bundle size.
...
We have moved the `Markdown` component to `fannypack-addon-markdown` to save some bundle size.
